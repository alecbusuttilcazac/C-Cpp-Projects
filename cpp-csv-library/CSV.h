...
#include <functional>
#include <optional>
#include "headers/CppTypeDef.h" // Project-specific typedefs
...
namespace CSV{    
    ...
    //   - Fatal: Process must terminate immediately
    //   - Non-Fatal: Can continue, but requires attention
    // Fatal exception: unrecoverable error
    class FatalException : public std::runtime_error{
        public:
            explicit FatalException(const std::string& errorMessage)
            :   std::runtime_error("FATAL CSV ERROR - " + errorMessage) {}
    };
    // Non-fatal exception: recoverable error
    ...
    class WriterClosedException : public FatalException{
        public:
    ...
    class ParseException : public FatalException{
        public:
            ...
            {}
            explicit ParseException(uint32 rowNumber, const std::string& filePath, std::string& errorMessage)
            :   FatalException("Parsing error at row number "+std::to_string(rowNumber)+".\n\t->\tFile: \""+filePath+"\"\n\t->\tAdditional Information: "+errorMessage)
            {}
    };
    
    class SchemaMismatchException : public FatalException{
        public:
        ...
        private:
            static std::string toString(const std::vector<std::string>& row, char delimiter){
                ...
                for(const auto& field : row){
                    ...
                    else returnString += delimiter + field;
                }
                return returnString += "]";
            }
    };
    ...
    class ReaderClosedException : public NonFatalException{
        public:
            explicit ReaderClosedException()
            :   NonFatalException("Reader functions inaccessible as writer is closed.")
            {}
    };
    class ReaderWriterClosedException : public NonFatalException{
        public:
    ...
    class NoHeaderException : public NonFatalException{
        public:
            explicit NoHeaderException(const std::string& filePath)
            :   NonFatalException("Header for "+filePath+" not set before calling header-related function.")
            {}
            explicit NoHeaderException(const std::string& filePath, const std::vector<std::string>& possibleHeader, char delimiter = ',')
    ...
    class ShortRowException : public NonFatalException{
        public:
    ...
    class InvalidLineException : public NonFatalException{
        public:
            explicit InvalidLineException(uint32 rowNumber)
            :   NonFatalException("Line "+std::to_string(rowNumber)+" exceeds file length.")
            {}
            ...
            :   NonFatalException("Line "+std::to_string(rowNumber)+" exceeds file length of "+std::to_string(numLinesInFile)+".\n\t->\tFile: \""+filePath+"\"")
            {}
    };
    
    // FileOpenFailureException: when file fails to open for reading/writing
    class FileOpenFailureException : public NonFatalException{
        public:
    ...
    class readRowException : public NonFatalException{
        public:
    ...
    class WriteLineException : public NonFatalException{
        public:
    ...
    class Reader{
        ...
        public:
            ...
            // Read all rows
            std::optional<std::vector<std::vector<std::string>>> readAll();
            std::optional<std::vector<std::vector<std::string>>> readAll(char delim);
    ...
    class Writer{
        ...
        public:
            ...
            void writeAll(const std::vector<std::vector<std::string>>& table); // Write all rows
            void writeAll(const std::vector<std::vector<std::string>>& table, char delim); // Write all rows with custom delimiter
...
//  === READER METHODS ===
    ...
    inline std::vector<std::string> Reader::getColumn(const std::string& columnName){
        if(!isOpen()) throw ReaderClosedException();
        if(!isHeaderSet()) throw FatalException("Header not set before calling getFieldByType()");
    ...
    inline std::string Reader::getFieldByType(const std::vector<std::string>& row, const std::string& columnName) const {
        if(!isOpen()) throw ReaderClosedException();
        if(!isHeaderSet()) throw NoHeaderException(path);
    ...
    inline std::optional<std::vector<std::vector<std::string>>> Reader::readAll(){
        if(!isOpen()) throw ReaderClosedException();
        uint32 originalLine = rowNumber;
        ...
        while (true) {
            auto maybeRow = readRow();
            if (!maybeRow) break;
            table.push_back(std::move(*maybeRow));
        }
        ...
        if(originalLine < table.size()) setRowNumber(originalLine);
        else rowNumber = table.size(); // Original position was past the data we read, stay at EOF
        
        if (table.empty()) return std::nullopt;
        return table;
    }
    
    inline std::optional<std::vector<std::vector<std::string>>> Reader::readAll(char delim){
        if(!isOpen()) throw ReaderClosedException();
        uint32 originalLine = rowNumber;
...
//  === WRITER METHODS ===
    ...
    if(!file.is_open()) throw FatalException("Writer is closed.");
        return rowNumber;
    }
    ...
    inline void Writer::writeAll(const std::vector<std::vector<std::string>>& table){
        if(!isOpen()) throw WriterClosedException();
        for(size_t i = 0; i < table.size(); ++i) writeRow(table[i]);
    }
    
    inline void Writer::writeAll(const std::vector<std::vector<std::string>>& table, char delim){
        if(!isOpen()) throw WriterClosedException();
        for(size_t i = 0; i < table.size(); ++i) writeRow(table[i], delim);
    }
...
//  === READERWRITER METHODS ===
    ...
    inline std::optional<std::vector<std::vector<std::string>>> ReaderWriter::readAll(){
        if(!isOpen()) throw ReaderWriterClosedException();
        return reader.readAll();
    }
    
    inline std::optional<std::vector<std::vector<std::string>>> ReaderWriter::readAll(char delim){
        if(!isOpen()) throw ReaderWriterClosedException();
    ...
    inline void ReaderWriter::writeAll(const std::vector<std::vector<std::string>>& table){
        if(!isOpen()) throw ReaderWriterClosedException();
    ...
    inline void ReaderWriter::writeAll(const std::vector<std::vector<std::string>>& table, char delim){
        if(!isOpen()) throw ReaderWriterClosedException();
        
        writer.writeAll(table, delim);
        reader.setNumLines(writer.getNumRows());
    }
